...
func tpmOpen(tpmPath string) (tpm.Closer, error) {
	...
	// For Windows, we'll only support the simulator for now
	// In a production environment, you would use the Windows TPM API
	if runtime.GOOS == "windows" {
		return nil, fmt.Errorf("TPM hardware access not supported on Windows in this example, use 'simulator' instead")
	}
	
	// This would work on Linux
	return nil, fmt.Errorf("TPM access not available on this platform")
}
